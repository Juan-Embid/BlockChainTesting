...
contract QuadraticVoting {
    ...
    uint256 public currentIndex; //  índice para saber dónde se interrumpió el bucle.
    bool public closing = false; // Estado para manejar el proceso de cierre.
...
// Modificador para asegurar que la votación esté abierta y en proceso de cierre.
    modifier isClosing() {
        require(votingOpen && closing, "Voting is not in the process of closing.");
        _;
    }

    function startClosingVoting() external onlyOwner {
        require(votingOpen, "Voting is already closed or not opened yet.");
        closing = true;
        currentIndex = 0;
    }
    function closeVoting() external onlyOwner isClosing {
        uint256 gasStart = gasleft(); // Gas al inicio de la operación.
        uint256 gasUsed; // Gas usado en cada iteración.

        // Procesar cada propuesta al cerrar la votación de forma incremental.
        for (; currentIndex < proposalCount && gasUsed < gasStart; currentIndex++) {
            Proposal storage proposal = proposals[currentIndex];
            if (!proposal.approved) {
                ...
                if (!proposal.executed) {
                    _checkAndExecuteProposal(currentIndex);
                }
            }
            // Limpia datos de votantes para liberar espacio en storage y evitar reentrancy issues
            delete proposal.voters;
            gasUsed = gasStart - gasleft(); // Calcula el gas usado en la iteración.
            gasStart = gasleft(); // Reset gasStart para la próxima iteración.
        }

        // Si el loop ha finalizado, cerrar completamente la votación.
        if (currentIndex >= proposalCount) {
            closing = false;
            votingOpen = false;
